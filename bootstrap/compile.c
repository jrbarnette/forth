...
x_colon(cell_ft tos, vmstate_p vm, addr_ft colon_xt)
{
    ...
    /* XXX : - parse name */
    ((xt_ft) allot(CELL_SIZE))->handler = colon_xt;
    return tos;
}
...
static cell_ft
x_literal(cell_ft tos, vmstate_p vm, addr_ft lit_xt)
{
    /* XXX LITERAL - enforce compilation state */
    CHECK_POP(vm, 1);
    *(a_addr_ft) allot(CELL_SIZE) = (cell_ft) lit_xt;
...
{
    a_addr_ft litptr = (a_addr_ft) vm->ip;
    CHECK_PUSH(vm, 1);
    PUSH(vm, tos);
    vm->ip = (xt_ft *)(litptr + 1);
    return *litptr;
}
