...
: start-errors 0 ;
: error:    1+ here swap parse-remainder counted, ;
: no-error  1+    0 swap ;
: error-table:
    ( maxerr ptrn ... ptr1 n -- )
    create here >r 2 cells allot dup >r 0 ?do , loop 1+ dup r> - r> 2!
    does> ( throw-code body -- c-addr u )
    2dup 2@ within if 2drop 0 0 exit then
    swap over @ - 2 + cells + @ ?dup if count exit then 0 0 ;
...
: .throw-code ans-error ?dup if type cr else drop then ;
